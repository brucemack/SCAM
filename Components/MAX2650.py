...
class MAX2650(Compound):
    ...
    def __init__(self, rotation_ccw=0):
        ...
        # Horizontal lines
        start_point = rotate((0.0, 2.0 * self.y_pitch_mm), rotation_ccw)
        end_point = rotate((2.0 * self.x_pitch_mm, 2.0 * self.y_pitch_mm), rotation_ccw)
        self.add(self.NO_OFFSET, Line(start_point, end_point))

        start_point = rotate((0, 1.0 * self.y_pitch_mm), rotation_ccw)
        end_point = rotate((4.0 * self.x_pitch_mm, 1.0 * self.y_pitch_mm), rotation_ccw)
        self.add(self.NO_OFFSET, Line(start_point, end_point))

        start_point = rotate((2.0 * self.x_pitch_mm, 0.0 * self.y_pitch_mm), rotation_ccw)
        end_point = rotate((4.0 * self.x_pitch_mm, 0.0 * self.y_pitch_mm), rotation_ccw)
        self.add(self.NO_OFFSET, Line(start_point, end_point))

        start_point = rotate((1.0 * self.x_pitch_mm, -1.0 * self.y_pitch_mm), rotation_ccw)
        end_point = rotate((2.0 * self.x_pitch_mm, -1.0 * self.y_pitch_mm), rotation_ccw)
        self.add(self.NO_OFFSET, Line(start_point, end_point))

        # Vertical lines
        start_point = rotate((0, 2.0 * self.y_pitch_mm), rotation_ccw)
        end_point = rotate((0.0 * self.x_pitch_mm, 1.0 * self.y_pitch_mm), rotation_ccw)
        self.add(self.NO_OFFSET, Line(start_point, end_point))

        start_point = rotate((1.0 * self.x_pitch_mm, 2.0 * self.y_pitch_mm), rotation_ccw)
        end_point = rotate((1.0 * self.x_pitch_mm, -1.0 * self.y_pitch_mm), rotation_ccw)
        self.add(self.NO_OFFSET, Line(start_point, end_point))

        start_point = rotate((2.0 * self.x_pitch_mm, 2.0 * self.y_pitch_mm), rotation_ccw)
        end_point = rotate((2.0 * self.x_pitch_mm, -1.0 * self.y_pitch_mm), rotation_ccw)
        self.add(self.NO_OFFSET, Line(start_point, end_point))

        start_point = rotate((3.0 * self.x_pitch_mm, 1.0 * self.y_pitch_mm), rotation_ccw)
        end_point = rotate((3.0 * self.x_pitch_mm, 0.0 * self.y_pitch_mm), rotation_ccw)
        self.add(self.NO_OFFSET, Line(start_point, end_point))

        start_point = rotate((4.0 * self.x_pitch_mm, 1.0 * self.y_pitch_mm), rotation_ccw)
        end_point = rotate((4.0 * self.x_pitch_mm, 0.0 * self.y_pitch_mm), rotation_ccw)
        self.add(self.NO_OFFSET, Line(start_point, end_point))

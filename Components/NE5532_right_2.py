...
class NE5532_right_2(Compound):
    ...
    def __init__(self, rotation_ccw=0):
        ...
        start_point = rotate((0, 2 * self.pitch_mm), rotation_ccw)
        end_point = rotate((self.width_mm, 2 * self.pitch_mm), rotation_ccw)
        self.add(self.NO_OFFSET, Line(start_point, end_point))

        start_point = rotate((0, 3 * self.pitch_mm), rotation_ccw)
        end_point = rotate((self.width_mm - 5, 3 * self.pitch_mm), rotation_ccw)
        self.add(self.NO_OFFSET, Line(start_point, end_point))
        ...
        start_point = rotate((self.width_mm - 5, 5 * self.pitch_mm), rotation_ccw)
        end_point = rotate((self.width_mm - 5, 3 * self.pitch_mm), rotation_ccw)
        self.add(self.NO_OFFSET, Line(start_point, end_point))
